...
void loop() {
  oledDraw(getSecond(), getTemp());  // draws the seconds as a clock and the temperature as a box
...
*Returns: void
*/
void oledDraw(int seconds, float temp) {
  int lineRadius = radius - 5;
  int newtemp = map(temp, 0, 40, 0, 64);

  float angle_rad = (seconds*6 - 90) * M_PI / 180.0;  // 0° points up
  int x = centerX + cos(angle_rad) * lineRadius;
  int y = centerY + sin(angle_rad) * lineRadius;

  u8g.firstPage();
  do {
    u8g.drawLine(centerX, centerY, x, y);
